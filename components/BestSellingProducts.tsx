...
//               height={400}
//               layout="responsive"
//               className="mb-4 w-full max-w-full h-auto max-h-60 rounded object-cover"
//             />
...
const BestSellingProducts: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setFavourites(storedFavourites.map((fav: Product) => fav.id));
  }, []);
  ...
  const handleFavouriteClick = (product: Product) => {
    ...
    if (isAlreadyFavorite) {
      ...
      localStorage.setItem("favourites", JSON.stringify(updatedFavourites));
      setFavourites(updatedFavourites.map((fav: Product) => fav.id));
      toast.error("Item removed from favourites", {
        position: "top-left",
        autoClose: 5000,
        theme: "dark",
          ...
          {currentProducts.map((product) => (
            ...
            >
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  onClick={() => handleCartClick(product)}
                  className="flex items-center rounded bg-blue-700 px-5 py-2 text-white transition duration-300 hover:bg-blue-800"
                >
                  <FaCartShopping className="mr-3" />
                  Add to cart
                </button>
